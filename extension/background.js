...
var updateContextMenu = function(tabId) {
  chrome.tabs.query({ active: true }, function(tab) {
    ...
    if (tab.id == tabId) {
      var index = registeredTabs.indexOf(tabId);
      if (index > -1) {
        chrome.contextMenus.update("keySocketMediaKeys-disableThisTab", {
          enabled: true
        });
        chrome.contextMenus.update("keySocketMediaKeys-enableThisTab", {
          enabled: false
        });
      } else {
        chrome.contextMenus.update("keySocketMediaKeys-disableThisTab", {
...
chrome.contextMenus.create({
  id: "keySocketMediaKeys-group",
  title: "Key Socket Media Keys"
});
