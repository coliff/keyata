...
keySocket.init("noonpacific", {
	"play-pause": function() {
		...
		for (var i = 0; i < playPauseButtonSelectors.length; i++) {
			var playPauseButtonCandidate = document.querySelector(
				playPauseButtonSelectors[i]
			);
			if (playPauseButtonCandidate) {
				keySocket.simulateClick(playPauseButtonCandidate, {
					cancelable: true
				});
				return;
			}
